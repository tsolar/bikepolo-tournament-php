...
class HamlView extends View {
	 ...
	 */
	var $defaults = array('ext'=>'.haml','useCachePath'=>true,'filePermission'=>0755,'cache'=>true, 'cache_location'=>'/tmp');
	/**#@-*/
   ...
   * 
	 ...
	 * @access public
	 */
	function element($name, $data = array(), $params = array(), $loadHelpers = false) {
...
//			$params = array_merge_recursive($params, $this->loaded);
			$element = $this->_render($file, array_merge($this->viewVars, $data), $loadHelpers);
	 ...
	 * @access protected 
	 */
	function _render($___viewFn, $___dataForView = array(), $loadHelpers = true, $cached = false) {
		$a = func_get_args();
	...
	private function _getCachedViewFileName($file)	{
		...
		if($this->useCachePath)	{
			$cachedViewFile = str_replace(APP . 'View', $this->cache_location, $cachedViewFile);
			if(!is_file($cachedViewFile) && !file_exists(dirname($cachedViewFile)))
