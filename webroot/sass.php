...
if(!empty($regs[1])){
	$pluginName = Inflector::camelize($regs[1]);
	$cssFile = APP . 'Plugin' . DS . $pluginName . DS . 'webroot' . DS . 'css' . DS . $regs[2];
} else {
	$cssFile = CSS . $regs[2];
}
...
//	App::import('Vendor', 'SassParser', array('file'=>'phamlp'.DS.'sass'.DS.'SassParser.php'));
	...
	$cnt = $parser->toCss($sassFile);
	if(Configure::read('debug')==0){
		file_put_contents(str_replace('/css/', '/ccss/', $cssFile), $cnt);
	}
	echo $cnt; 
	die();
}
